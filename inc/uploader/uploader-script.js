...
/*-----------------------------------------------------------------------------------*/
	...
	removeFile: function () {
		$(document).on( 'click', '.a3_uploader_remove', function(event) {
			...
			$(this).parents().parents().children('.a3_upload_attachment_size').val('');
			...
			*/
			...
			file_frame.on( 'select', function() {
				...
				if (attachment.url.match(image)) {
					...
					html = '<a href="'+attachment.url+'" target="_blank" rel="noopener" rel="a3_external">View File</a>';
				...
				$( '#' + formfield + '_attachment_size').val(size);
